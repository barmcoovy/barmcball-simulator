...
function App() {
  ...
  const handleGameStart = (position, nationality, club) => {
    generateRandomAttributes(position);
  ...
  const generateRandomAttributes = (position) => {
    ...
    switch (position) {
      ...
      case "LW":
        return {
          shooting: random(55, 65),
          passing: random(50, 60),
      ...
      <section className="flex flex-col items-center  min-h-screen bg-linear-to-b from-green-900 to-black">
        ...
        {gameStatus === "clubChoice" && selectedClub !== null && (
          <button
            ...
            className="bg-green-600 hover:bg-green-500 text-white font-semibold py-2 px-4 rounded-lg transition cursor-pointer"
          >
            Start Game
          </button>
        )}
